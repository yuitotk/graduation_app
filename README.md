...
5. ストーリーイベント管理
6. 要素管理（キャラクター／アイテム／設定／場所／団体・グループ、絵文字などの短いマーカー付き）
7. イベント×要素の多対多紐づけ
8. イベント内の詳細メモ管理
9. 整合性チェック（選んだ要素が関わるイベントだけを時系列で表示）
10. 検索（アイデアのタイトル・メモをLIKE検索、サジェスト付き）
11. ユーザー認証（新規登録／ログイン／ログアウト／パスワードリセット／Googleアカウント連携ログイン）
12. ゲスト利用（登録なしで試せる。ストーリー作成数・AI生成回数に制限があり、あとから正式アカウントに切り替え可能）
13. 見本ストーリー・見本アイデア（新規登録・ゲスト開始時に、使い方を確認できる見本データを自動で用意。
    通常のデータと色分けして表示し、削除も可能）
14. お問い合わせ（入力→確認→完了）
15. クイックアクションボタン（画面右下の⚡から、アイデア作成・AI作成・要素管理・
    整合性チェック・イベント追加・詳細メモ追加にどのページからでもすぐ移動できる）

## 使い方の流れ

1. トップページ（`/`）から新規登録（メール＋パスワード、またはGoogleアカウント）、ログイン、
   もしくは「ゲストとして始める」（登録なしで試用、一部機能に制限あり）
2. ログイン後は「アイデア一覧」（`/ideas`）がホーム画面。配置先が未設定のアイデアが並ぶ。
   新規登録・ゲスト開始をした時点で、ジャンル違いの見本アイデアと見本ストーリーが最初から入っている（削除可）
...
5. 「要素」としてキャラクター・アイテム・設定・場所・団体/グループを登録し、イベントに紐づけ
...
| アプリケーションサーバー | Puma |
| 認証 | sorcery（パスワードリセット・Google外部連携ログイン） |
...
- **Google OAuth**：Googleアカウントでのログイン連携
